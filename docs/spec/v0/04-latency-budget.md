...
7. **End-to-end integration test**: play a 30s CN speech file → measure time from audio-out to first audible TTS in BlackHole. Assert ≤3000 ms on the wired cn-north network. **This is the canary.** Plus a **network drop + reconnect variant (per D30-Q3, locked 2026-09-07, `docs/decisions/round-3-confirmations.md` D30-Q3)**: drop WS for 3s mid-R3 session (use `tc qdisc` or kill `tokio-tungstenite` task); measure (a) reconnect completes within ≤7s; (b) first post-reconnect `TTSResponse(352)` cosine voice-embedding similarity ≥0.85 to chunk emitted 5s before drop (per `voice-clone-strategies.md` §8.3); (c) zero audio frames lost between drop and first post-reconnect chunk. Strategy A = 10s replay ring (crossbeam SPSC buffer, last 10s of user audio replayed on reconnect so API re-extracts voice profile from continuous audio). Strategy F = exponential backoff 1s/2s/4s, max 3 retries, reset counter on `SessionStarted(150)` (avoids Doppelvoice v0.2.2 30s lockout bug per `voice-clone-strategies.md` §8.6).
